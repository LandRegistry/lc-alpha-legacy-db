...
def retrieve_image(app, regn_no, index):
    filename, mimetype = find_file(app.config['IMAGE_DIRECTORY'], regn_no, index)
    if filename is None:
        return None
    return send_from_directory(os.path.dirname(filename), os.path.basename(filename))


def create_update_image(app, regn_no, index):
    ...
    filename = '{}img{}_{}.{}'.format(app.config['IMAGE_DIRECTORY'], regn_no, index, extn)
...
def remove_image(app, regn_no, index):
    filename, mimetype = find_file(app.config['IMAGE_DIRECTORY'], regn_no, index)
    if filename is None:
        return None
    os.remove(filename)
...
def find_file(directory, regn_no, index):
    filename_base = '{}img{}_{}'.format(directory, regn_no, index)
    filenames = [
